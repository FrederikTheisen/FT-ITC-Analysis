...
public static class InterpretationAdminCommands
{
    ...
    static int Presets(string[] args,GenerationPresetRegistry registry,TextWriter output,TextWriter error)
    {
        ...
        if(args.Length==3&&args[0]=="set-access")
        {
            ...
            var value = registry.UpdateAccess(args[1], tiers);
            output.WriteLine($"preset={args[1]} tiers={string.Join(',', value.Presets.Single(x => x.Id == args[1]).AllowedTiers)} revision={value.Revision}");
            return 0;
        }
        return Help(error);
    }
