...
coefficient arrays. Use `canonical-package.json` when those omitted control
...
The collection-capable client requires the matching FT-ITC interpretation service using relay contract 5.0. The **Summary** generation task produces a compact factual report without retrieval or capability-code quota usage. There is no compatibility adapter or version negotiation; update the app and MIST service together. Local report editing, saved interpretations and PDF export remain available if generation is unavailable.
