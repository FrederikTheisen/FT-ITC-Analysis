...
Interpretation troubleshooting: the application log records generation stages, request IDs, prompt versions and fingerprints, package sizes, timing, HTTP status and rejected contract fields. Full prompts, scientific context, generated text and credentials are not included in these diagnostic entries. Include the application log and visible error when reporting a generation failure. The desktop package contract and server must be kept compatible even when their major version labels match.

The returned text is a draft.

Use **Save AI package…** in the generation dialog to save a local ZIP with the complete evidence JSON, local prompt text and a version/fingerprint manifest. It uses the current selection, question, context and thermogram setting without a server request or new fits. This is a snapshot before transport; server prompt updates, retrieval and fallback omissions are not captured. The archive contains your scientific data and context, so review it before sharing.
