...
three, or four steps are active. The selected constraint applies to every
active step while keeping a separate parameter for each step: **Same for
...
Display settings affect the graph presentation, not the model, the member data, or the fitted values. The graph can therefore show the independently fitted curves or global fit alongside each experiment's observations while the Parameters tab remains the source of the stored parameter and constraint state.
