...
schema version `2` (historical heat), `3` (Dumas heat), or `4` (Discrete displacement heat). It must contain the explicit integer model option
...
Result metadata stores the global-solution ID, global validity, ordered member-solution IDs, model, constraints, global parameters, clone options, convergence, and a historical fit-input validity snapshot. Global results also optionally store `referenceTemperatureKelvin`, the fixed origin used for temperature relationships; older files derive it once from their member temperatures. Constraint values use `none`, `temperature-dependent`, `same-for-all`, or `thermodynamically-linked`; the historical temperature-dependent value retains its existing shared-ΔG meaning. The snapshot retains fit-time corrected heats so stale results can still be diagnosed. Optional `memberTemperaturesKelvin` maps member solution IDs to captured fit-time temperatures. Optional `globalReplicates` records each global refit's `index`, participating `memberSolutionIds`, and `globalParameters` using the existing parameter representation. Local values and temperatures come from the matching member bootstrap snapshots. Refits inherit the parent constraints and fixed reference; explicit membership includes leave-one-out subsets. Readers retain the primary result when recovery discards malformed global replicate metadata. Without these fields, the existing common-index reconstruction remains available; loading does not refit missing coordinates.
...
Package schema remains 1.6/project schema 4. Experiments, bootstrap replicate descriptors and historical validity-experiment snapshots optionally store `concentrationMethod` (`microcal`, `exponential`, or `pytc-discrete`) and `heatMethod` (`legacy`, `dumas-simpson`, or `pytc-discrete`). Missing/null concentration metadata means the saved concentration law is unknown. Missing/null heat metadata means historical behavior; it never opts into Dumas or Discrete displacement. Dumas experiment/replicate state requires explicit `exponential` concentration metadata. The `pytc-discrete` concentration and heat methods must occur together. Unknown wire identifiers are rejected. Loading retains stored concentrations without recalculation.

**Discrete displacement** is the UI and implementation name for the method originally labelled pytc. Its stable `pytc-discrete` wire identifiers and numeric preference value `2` are unchanged; renaming requires no file migration or schema change.
...
A missing heat method with a Dumas/Discrete displacement schema, or another mismatched schema/method combination, is rejected. Older readers consequently reject unsupported fits instead of evaluating them as historical models. Recovery can omit an unsupported fit while retaining usable experimental data where the experiment metadata is understood. This is not a guarantee that older software can read or refit newer-method data correctly.
