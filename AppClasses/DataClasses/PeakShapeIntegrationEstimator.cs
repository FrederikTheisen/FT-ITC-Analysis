...
namespace AnalysisITC.Core.Data
{
    internal static class PeakShapeIntegrationEstimator
    {
        ...
        public static float EstimateEndOffset(ExperimentData experiment, InjectionData injection, float factor)
        {
            ...
            if (idx < 0)
                idx = FWEMath.Clamp(injection.ID, 0, offsets.Length - 1);
        ...
        static float EstimateEndOffset(InjectionData injection, PeakShapeDescriptor descriptor, double globalTau, double dt, float factor)
        {
            ...
            if (globalTau > dt)
            {
                tau = FWEMath.Clamp(tau, MinTauScaleVsGlobal * globalTau, MaxTauScaleVsGlobal * globalTau);
        ...
        static PeakShapeDescriptor AnalyzeInjection(ExperimentData experiment, InjectionData injection)
        {
            try
            {
                ...
                double preWindow = FWEMath.Clamp(injection.Delay * PreWindowFractionOfDelay, MinPreWindowSeconds, MaxPreWindowSeconds);
                double postWindow = FWEMath.Clamp(injection.Delay * PostWindowFractionOfDelay, MinPostWindowSeconds, MaxPostWindowSeconds);
        ...
        static double GetLocalTauWeight(double signalToNoise)
        {
            ...
            return FWEMath.Clamp((signalToNoise - 3.0) / 7.0, 0.2, 0.85);
        }
